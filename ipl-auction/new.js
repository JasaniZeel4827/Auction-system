...
function renderPlayers() {
    ...
    players.forEach((player, index) => {
        ...
        playerDetails.textContent = `${index + 1}. ${player.player} - ${player.teamName} - ${
            player.category
        } - Base Price: ₹${player.basePrice} Crores`;
...
function updateTeamBudget(teamId, budget) {
    document.getElementById(`budget-${teamId}`).textContent = `₹${budget} Crores`;
}
...
function teamBid(teamId) {
    const bidAmount = parseFloat(
        prompt(
            `Enter bidding amount for ${players[playerIndex].player}:`,
...
function sellPlayer() {
    ...
    if (highestBidder !== null) {
        ...
        soldTo.textContent = `SOLD to: ${teams[teamId].name} for ₹${bidAmount} Crores`;
        ...
        purchasedItem.textContent = `${player.player} - ₹${bidAmount} Crores`;
